...
export default function Credits() {
  ...
  const accordionData = [
    ...
    },
    {
      ...
      details: "Major: (self-designed) Graphic & Interactive Design for Art & Business",
      content: "For her senior capstone project, Kelly took everything that Philip and Dr. Cermak had built and designed a clean, modern user interface prioritizing the best user experience for various audiences and objectives. She also worked closely with Dr. Cermak to build and refine the technical prose and illustrations. She conducted user experience research and testing along the way, reported research findings, and developed countless design iterations to get to what we have before us today. She also recruited and managed all of the computer science students in an Agile work environment.",
  ...
  return (
    <main className="bg-[#0C0C31] w-full h-full p-10 text-white">
      ...
      <div className="flex gap-10">
        <div className="w-1/2">
          <img src={imageSrc} alt="Project Visual" className="h-[840px] w-full rounded-lg shadow-lg object-contain" />
        </div>
        ...
        <div className="w-1/2 flex flex-col gap-10 items-start justify-evenly">
          <div className="left-[1050px] font-serif text-[42px] text-[#FEFCFC] overflow-y-auto px-4 scrollbar-custom flex flex-col gap-8 h-[600px]">
            {accordionData.map((item, index) => (
              <div key={index} className="border-b border-gray-500 pb-4">
                ...
                >
                  {item.title}
                  <span className="text-[36px]">{openIndex === index ? "▲" : "▼"}</span>
                </button>
                <motion.div
          ...
          <div className="flex gap-10 items-left w-full self-end justify-self-end">
            <div className="flex items-center text-[#A5C882] text-[36px] font-bold justify-self-start">
              <Link to="/about-project" className="flex items-center gap-2 hover:text-white transition-all duration-300">
